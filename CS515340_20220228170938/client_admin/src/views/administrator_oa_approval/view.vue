		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','management_employee_number') || $check_field('add','management_employee_number') || $check_field('set','management_employee_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="管理员工号" prop="management_employee_number">
						<div v-if="user_group !== '管理员'">
							{{ get_user_session_management_employee_number(form['management_employee_number']) }}
							<!--<el-input id="business_name" v-model="form['management_employee_number']" placeholder="请输入管理员工号"-->
							<!--v-if="user_group === '管理员' || (form['administrator_oa_approval_id'] && $check_field('set','management_employee_number')) || (!form['administrator_oa_approval_id'] && $check_field('add','management_employee_number'))" :disabled="disabledObj['management_employee_number_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','management_employee_number')">{{form['management_employee_number']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['administrator_oa_approval_id'] && $check_field('set','management_employee_number')) || (!form['administrator_oa_approval_id'] && $check_field('add','management_employee_number'))" id="management_employee_number" v-model="form['management_employee_number']" :disabled="disabledObj['management_employee_number_isDisabled']">
						...
						<el-select v-else id="management_employee_number" v-model="form['management_employee_number']" :disabled="disabledObj['management_employee_number_isDisabled']">
							<el-option v-for="o in list_user_management_employee_number" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
									   :value="o['user_id']">
							</el-option>
						</el-select>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','approval_type') || $check_field('add','approval_type') || $check_field('set','approval_type')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="审批类型" prop="approval_type">
					<el-select id="approval_type" v-model="form['approval_type']"
						v-if="user_group === '管理员' || (form['administrator_oa_approval_id'] && $check_field('set','approval_type')) || (!form['administrator_oa_approval_id'] && $check_field('add','approval_type'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','applicant') || $check_field('add','applicant') || $check_field('set','applicant')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="申请人" prop="applicant">
					<el-input id="applicant" v-model="form['applicant']" placeholder="请输入申请人"
							  v-if="user_group === '管理员' || (form['administrator_oa_approval_id'] && $check_field('set','applicant')) || (!form['administrator_oa_approval_id'] && $check_field('add','applicant'))" :disabled="disabledObj['applicant_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','approval_status') || $check_field('add','approval_status') || $check_field('set','approval_status')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="审批情况" prop="approval_status">
					<el-select id="approval_status" v-model="form['approval_status']"
						v-if="user_group === '管理员' || (form['administrator_oa_approval_id'] && $check_field('set','approval_status')) || (!form['administrator_oa_approval_id'] && $check_field('add','approval_status'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','approver_reply') || $check_field('add','approver_reply') || $check_field('set','approver_reply')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="审批人回复" prop="approver_reply">
					<el-input id="approver_reply" v-model="form['approver_reply']" placeholder="请输入审批人回复"
							  v-if="user_group === '管理员' || (form['administrator_oa_approval_id'] && $check_field('set','approver_reply')) || (!form['administrator_oa_approval_id'] && $check_field('add','approver_reply'))" :disabled="disabledObj['approver_reply_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','approver') || $check_field('add','approver') || $check_field('set','approver')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="审批人" prop="approver">
					<el-input id="approver" v-model="form['approver']" placeholder="请输入审批人"
							  v-if="user_group === '管理员' || (form['administrator_oa_approval_id'] && $check_field('set','approver')) || (!form['administrator_oa_approval_id'] && $check_field('add','approver'))" :disabled="disabledObj['approver_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','date') || $check_field('add','date') || $check_field('set','date')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="日期" prop="date">
					<el-date-picker :disabled="disabledObj['date_isDisabled']" v-if="user_group === '管理员' || (form['administrator_oa_approval_id'] && $check_field('set','date')) || (!form['administrator_oa_approval_id'] && $check_field('add','date'))" id="date"
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','approval_content') || $check_field('add','approval_content') || $check_field('set','approval_content')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="审批内容" prop="approval_content">
					<el-input type="textarea" id="approval_content" v-model="form['approval_content']" placeholder="请输入审批内容"
						v-if="user_group === '管理员' || (form['administrator_oa_approval_id'] && $check_field('set','approval_content')) || (!form['administrator_oa_approval_id'] && $check_field('add','approval_content'))" :disabled="disabledObj['approval_content_isDisabled']"></el-input>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "administrator_oa_approval_id",
				url_add: "~/api/administrator_oa_approval/add?",
				url_set: "~/api/administrator_oa_approval/set?",
				url_get_obj: "~/api/administrator_oa_approval/get_obj?",
				url_upload: "~/api/administrator_oa_approval/upload?",

				query: {
					"administrator_oa_approval_id": 0,
				},
				...
				form: {
					...
					"administrator_oa_approval_id": 0, // ID
				...
				// 用户组
				group_user_management_employee_number: "",
				//审批类型选项列表
		...
		methods: {
			 ...
			 * 获取管理层用户用户组
			 */
			async get_group_user_management_employee_number() {
				this.form["management_employee_number"] = this.user.user_id;
				var json = await this.$get("~/api/user_group/get_obj?name=管理层用户");
				if(json.result && json.result.obj){
					this.group_user_management_employee_number = json.result.obj;
				}
				else if(json.error){
					console.error(json.error);
				}
			},
			get_user_session_management_employee_number(id){
				var _this = this;
				var user_id = {"user_id":id}
				var url = "~/api/"+_this.group_user_management_employee_number.source_table+"/get_obj?"
					...
					if (res.result && res.result.obj) {
						...
						for (var i=0;i<arr.length;i++){
							for (var j=0;j<arrForm.length;j++){
								if (arr[i]===arrForm[j]){
									if (arr[i]!=="management_employee_number") {
        ...
        if (this.form["date"].indexOf("-")===-1){
          this.form["date"] = this.$toTime(parseInt(this.form["date"]),"yyyy-MM-dd")
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/administrator_oa_approval/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/administrator_oa_approval/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/administrator_oa_approval/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/administrator_oa_approval/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/administrator_oa_approval/view','get');
		...
		created() {
			...
			this.get_group_user_management_employee_number();
		}
	}
</script>
