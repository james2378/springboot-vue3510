		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','task_title') || $check_field('add','task_title') || $check_field('set','task_title')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="任务标题" prop="task_title">
					<el-input id="task_title" v-model="form['task_title']" placeholder="请输入任务标题"
							  v-if="user_group === '管理员' || (form['management_task_management_id'] && $check_field('set','task_title')) || (!form['management_task_management_id'] && $check_field('add','task_title'))" :disabled="disabledObj['task_title_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','management_employee_number') || $check_field('add','management_employee_number') || $check_field('set','management_employee_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="管理员工号" prop="management_employee_number">
						<el-select v-if="user_group === '管理员' || (form['management_task_management_id'] && $check_field('set','management_employee_number')) || (!form['management_task_management_id'] && $check_field('add','management_employee_number'))" id="management_employee_number" v-model="form['management_employee_number']" :disabled="disabledObj['management_employee_number_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form['management_task_management_id'] && $check_field('set','full_name')) || (!form['management_task_management_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','task_status') || $check_field('add','task_status') || $check_field('set','task_status')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="任务状态" prop="task_status">
					<el-select id="task_status" v-model="form['task_status']"
						v-if="user_group === '管理员' || (form['management_task_management_id'] && $check_field('set','task_status')) || (!form['management_task_management_id'] && $check_field('add','task_status'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','closing_date') || $check_field('add','closing_date') || $check_field('set','closing_date')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="截止日期" prop="closing_date">
					<el-date-picker :disabled="disabledObj['closing_date_isDisabled']" v-if="user_group === '管理员' || (form['management_task_management_id'] && $check_field('set','closing_date')) || (!form['management_task_management_id'] && $check_field('add','closing_date'))" id="closing_date"
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','task_details') || $check_field('add','task_details') || $check_field('set','task_details')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="任务详情" prop="task_details">
					<el-input type="textarea" id="task_details" v-model="form['task_details']" placeholder="请输入任务详情"
						v-if="user_group === '管理员' || (form['management_task_management_id'] && $check_field('set','task_details')) || (!form['management_task_management_id'] && $check_field('add','task_details'))" :disabled="disabledObj['task_details_isDisabled']"></el-input>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "management_task_management_id",
				url_add: "~/api/management_task_management/add?",
				url_set: "~/api/management_task_management/set?",
				url_get_obj: "~/api/management_task_management/get_obj?",
				url_upload: "~/api/management_task_management/upload?",

				query: {
					"management_task_management_id": 0,
				},
				...
				form: {
					...
					"closing_date":'', // 截止日期
					"task_details":'', // 任务详情
					"management_task_management_id": 0, // ID
        ...
        if (this.form["closing_date"].indexOf("-")===-1){
          this.form["closing_date"] = this.$toTime(parseInt(this.form["closing_date"]),"yyyy-MM-dd")
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/management_task_management/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/management_task_management/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/management_task_management/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/management_task_management/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/management_task_management/view','get');
