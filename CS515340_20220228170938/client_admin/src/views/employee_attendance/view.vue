		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','job_number') || $check_field('add','job_number') || $check_field('set','job_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="工号" prop="job_number">
						<div v-if="user_group !== '管理员'">
							...
							<!--v-if="user_group === '管理员' || (form['employee_attendance_id'] && $check_field('set','job_number')) || (!form['employee_attendance_id'] && $check_field('add','job_number'))" :disabled="disabledObj['job_number_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','job_number')">{{form['job_number']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['employee_attendance_id'] && $check_field('set','job_number')) || (!form['employee_attendance_id'] && $check_field('add','job_number'))" id="job_number" v-model="form['job_number']" :disabled="disabledObj['job_number_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form['employee_attendance_id'] && $check_field('set','full_name')) || (!form['employee_attendance_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','sign_in') || $check_field('add','sign_in') || $check_field('set','sign_in')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="签到" prop="sign_in">
					<el-time-picker v-if="user_group === '管理员' || (form['employee_attendance_id'] && $check_field('set','sign_in')) || (!form['employee_attendance_id'] && $check_field('add','sign_in'))" id="sign_in"
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','sign_out') || $check_field('add','sign_out') || $check_field('set','sign_out')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="签退" prop="sign_out">
					<el-time-picker v-if="user_group === '管理员' || (form['employee_attendance_id'] && $check_field('set','sign_out')) || (!form['employee_attendance_id'] && $check_field('add','sign_out'))" id="sign_out"
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','attendance_status') || $check_field('add','attendance_status') || $check_field('set','attendance_status')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="出勤状态" prop="attendance_status">
					<el-select id="attendance_status" v-model="form['attendance_status']"
						v-if="user_group === '管理员' || (form['employee_attendance_id'] && $check_field('set','attendance_status')) || (!form['employee_attendance_id'] && $check_field('add','attendance_status'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','date') || $check_field('add','date') || $check_field('set','date')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="日期" prop="date">
					<el-date-picker :disabled="disabledObj['date_isDisabled']" v-if="user_group === '管理员' || (form['employee_attendance_id'] && $check_field('set','date')) || (!form['employee_attendance_id'] && $check_field('add','date'))" id="date"
