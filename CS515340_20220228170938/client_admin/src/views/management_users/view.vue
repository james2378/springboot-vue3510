	...
	<el-main class="bg">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','job_number') || $check_field('add','job_number') || $check_field('set','job_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="管理员工号" prop="job_number">
					<el-input id="job_number" v-model="form_sub['job_number']" placeholder="请输入管理员工号"
							  v-if="user_group === '管理员' || (form_sub['management_users_id'] && $check_field('set','job_number')) || (!form_sub['management_users_id'] && $check_field('add','job_number'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','department') || $check_field('add','department') || $check_field('set','department')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="部门" prop="department">
					<el-input id="department" v-model="form_sub['department']" placeholder="请输入部门"
							  v-if="user_group === '管理员' || (form_sub['management_users_id'] && $check_field('set','department')) || (!form_sub['management_users_id'] && $check_field('add','department'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','full_name') || $check_field('add','full_name') || $check_field('set','full_name')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="姓名" prop="full_name">
					<el-input id="full_name" v-model="form_sub['full_name']" placeholder="请输入姓名"
							  v-if="user_group === '管理员' || (form_sub['management_users_id'] && $check_field('set','full_name')) || (!form_sub['management_users_id'] && $check_field('add','full_name'))"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','gender') || $check_field('add','gender') || $check_field('set','gender')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="性别" prop="gender">
					<el-select id="gender" v-model="form_sub['gender']"
						v-if="user_group === '管理员' || (form_sub['management_users_id'] && $check_field('set','gender')) || (!form_sub['management_users_id'] && $check_field('add','gender'))">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','title') || $check_field('add','title') || $check_field('set','title')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="职称" prop="title">
					<el-input id="title" v-model="form_sub['title']" placeholder="请输入职称"
							  v-if="user_group === '管理员' || (form_sub['management_users_id'] && $check_field('set','title')) || (!form_sub['management_users_id'] && $check_field('add','title'))"></el-input>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["management_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?management_users_id=" + form_sub["management_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/management_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/management_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/management_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/management_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/management_users/view','get');
